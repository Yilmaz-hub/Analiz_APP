# Spec 0002 — İşlem Tutarlılığı Uygulama Planı
...
**Spec:** `specs/0002-islem-kararlari-ve-performans-tutarliligi.md`, Revizyon 18; R01–R30 ve 104 kabul kriteri (AC14–AC20, AC26–AC122).
...
- AK01–AK04 ve AK06 kapalıdır. Yalnız AK05 sayısal süre hedefi açıktır.
...
AK01–AK03 onaylıdır. AK05 ölçüm yöntemi onaylı, sayısal hedefleri açıktır. 0003 numarası ve eski/teyitsiz kayıt geçişi onaylı hazırlık kararlarıdır. Uygulama planının tamamı onay bekler.
...
- `docs/evidence/0002/` — kriter numarasıyla ekran görüntüleri, ölçüm koşulları ve test raporları; uygulama doğrulaması sırasında oluşturulur.

`ml_models.py`, `advanced_analysis.py`, `optimize_weights.py` için strateji değişikliği planlanmıyor. Ortak değerlendirmeye uyarlama zorunluluğu keşfedilirse kapsam ve gerekçe ayrıca sunulur; ML sessizce kapatılmaz.
...
- [ ] Mevcut çalışma kopyasını koruyarak `feature/0002-islem-tutarliligi` için izolasyon hazırla; ancak uygulama onayından sonra.
...
Dosyalar: `tests/test_trading_performance.py`, `.github/workflows/tests.yml`, koşullu `requirements-dev.txt`, `docs/evidence/0002/`.
...
Bu plan testlerin çalıştığı veya geçtiği iddiası değildir. Yalnız plan belgesi oluşturuldu. Uygulama için Takım Yöneticisinin plan onayı beklenir; açık AK kararları bağımlı dilimlerin başlangıç koşulu olarak kalır.
