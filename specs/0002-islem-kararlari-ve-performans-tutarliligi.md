# Spec: 0002 — İşlem Kararları ve Performans Tutarlılığı (V1)

> Şablon: [TEMPLATE.md](TEMPLATE.md). Bitince → `specs/done/`.
> Rol: Analist — INTENT · CLARIFY · SPEC.
> Durum: Revizyon 18; toplu CLARIFY kararları ve USD/USDT eşdeğerliği onaylıdır. Yalnız AK05 sayısal süre hedefleri açıktır. Uygulama planı onay bekler.
...
AK01–AK04 ve AK06 kapalıdır. Yalnız AK05 sayısal süre hedefi açıktır; ölçüm protokolü onaylıdır.
...
### AK05 — Performans: ölçüm açık kaldı

Yerel tanı denemesinde Python 3.14.2 görüldü; CI Python 3.13 kullanıyor. Veri işleme modülünün yüklenmesi tamamlanmadığından deneme kesildi. Sinyal ve geçmiş test süreleri ölçülemedi. Bu gözlem uygulamanın normal kullanıcı ortamındaki yavaşlık nedeni hakkında kesin teşhis değildir.

**Onaylı ölçüm yöntemi:** CI ile aynı Python 3.13 ortamında, aynı bağımlılık sürümleriyle ölçüm yapılsın. Tek varlık/500 günlük mum ekran hesabı, tek varlık/1.000 günlük mum geçmiş testi ve dört piyasa grubunu içeren 20 varlıklı tarama ayrı ölçülsün. ML açık kalsın. Mevcut 500 mumluk sabit test verisi başlangıç karşılaştırması için kullanılsın; her piyasanın gerçek takvimini temsil ettiği iddia edilmesin.

Beş bağımsız ilk açılış ve üç hazırlık çalıştırmasından sonra yirmi tekrar için medyan, p95 ve en uzun süre raporlansın. Ağ hariç hesaplama ile ağ/ilk yükleme/ekran oluşumu dahil toplam bekleme ayrı tutulsun. Sağlayıcı hataları ve zaman aşımları başarı ölçümlerinden saklanmasın. Donanım ve veri kesiti rapora yazılsın. Mutlak saniye bütçeleri bu ölçümden sonra tek öneride onaya sunulsun; şu an sayı vermek kanıta dayalı olmaz.

**Gerekçe:** Bir önbellek tekrarını tüm uygulamanın performansı gibi sunmayı ve yerel ortam farkını ürüne ait sonuç sanmayı önler. Paket onaylanmıştır; AK05'in sayısal bütçesi açık kalır.
...
| Metrik | Değer |
|--------|-------|
| Spec revizyon sayısı | 17 — miktar adımı ve kalan açık başlık envanteri işlendi |
